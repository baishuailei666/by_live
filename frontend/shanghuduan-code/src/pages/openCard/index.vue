  ...
  <div class="service-card">
    ...
    <el-dialog title="微信扫码签署合同"
      ...
      <div style="text-align:center;line-height:22px;">
        ...
        <div style="margin-bottom:20px;color:#ff2525;">当前已选个人签/企业签， 未签署将无法进行支付。</div>
        <div class="dianziqianlogo">
        </div>
      </div>
...
export default {
  ...
  created () {
    this.priceDataBox = JSON.parse(localStorage.getItem('priceData'))
  ...
  methods: {
    ...
    async confirmFrom () {
      ...
      if (res.code == 200) {
        ...
        let res1 = await signCreate(params1)
        if (res1.code == 200) {
          // console.log(res1)
    ...
    async immediatelyOpenBtn () {
      ...
      let res1 = await shop()
      // console.log(res1)
      if (res1.data.auditStatus != '已通过') {//点击已阅读并同意之后再进行跳转到“收银台”
...
.service-card .el-button:hover {
  opacity: 0.8;
  border-color: #ffdebd;
  box-shadow: 0 3px 8px 0 rgb(255, 156, 57, 61%);
}
.service-card .contractTipsBtn1 {
  background-image: none;
  background-color: rgb(204, 204, 204) !important;
  color: #fff;
  border: none;
}
.service-card .contractTipsBtn1:hover {
  opacity: 1;
  border-color: none;
  box-shadow: none;
  color: #fff;
